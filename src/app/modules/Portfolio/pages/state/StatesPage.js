...
export function StatesPage({ history }) {
  ...
  const companiesUIEvents = {
    newCompanyButtonClick: () => {
      history.push("/portfolio/states/new");
    },
     openEditCompanyDialog: (companyMasterId) => {
      history.push(`/portfolio/states/${companyMasterId}/edit`);
    },
 ...
 {/*  <CompaniesLoadingDialog />  */}
       <Route path="/portfolio/states/new">
        {({ history, match }) => (
          <CompanyEditDialog
            show={match != null}
            onHide={() => {
              history.push("/portfolio/states");
            }}
          />
        )}
      </Route> 
      <Route path="/portfolio/states/:stateMasterId/edit">
        {({ history, match }) => (
          <CompanyEditDialog
            show={match != null}

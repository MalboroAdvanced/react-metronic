...
import {
  PortfoliosStatusCssClasses,
  PortfoliosStatusTitles,
...
const selectedPortfolios = (entities, ids) => {
  const _portfolios = [];
  ids.forEach((portfolioMasterId) => {
    const portfolio = entities.find((el) => el.portfolioMasterId === portfolioMasterId);
    if (portfolio) {
        ...
        <table className="table table table-head-custom table-vertical-center overflow-hidden">
          ...
          <tbody>
            {portfolios.map((portfolio) => (
              <tr key={`portfolioMasterId${portfolio.portfolioMasterId}`}>
                <td>{portfolio.portfolioMasterId}</td>
                <td>
                  <span
                    className={`label label-lg label-light-${
                      PortfoliosStatusCssClasses[portfolio.status]
                    } label-inline`}
                  >
                    {" "}
                    {PortfoliosStatusTitles[portfolio.status]}
                  </span>
                </td>
                <td>
                  <span className="ml-3">

...
import {
  AVAILABLE_DISCOMTYPES,AVAILABLE_POINT_PER, AVAILABLE_POINT_OF_CONNECTION
...
// Validation schema
const DiscomEditSchema = Yup.object().shape({
  discomName: Yup.string()
    .min(3, "Minimum 3 symbols")
    .max(50, "Maximum 50 symbols")
    .required("Discom Name is required"),
    discomId: Yup.string()
    .min(3, "Minimum 3 symbols")
    .max(50, "Maximum 50 symbols")
    .required("Discom Id is required"),
...
export function DiscomEditForm({
  saveDiscom,
  getRegionForState,
  discom,
  actionsLoading,
  ...
  return (
    <>
      <Formik
        ...
        initialValues={discom}
        validationSchema={DiscomEditSchema}
        onSubmit={(values) => {
          saveDiscom(values);
        }}
      >
              ...
              <div className="form-group row">
               ...
               <States getRegion={getRegion}/>
                ...
                <Select name="regionName" label="Region"  >
                 {/* 
                  {AVAILABLE_DISCOMTYPES.map((regionName) => (
                ...
                <Select name="perConnection" label="Connection Point/Per">
                  ...
                  <div className="col-lg-4">
                <Select name="discomType" label="Discom Type">
                 
                    {AVAILABLE_DISCOMTYPES.map((discomType) => (
                      <option key={discomType} value={discomType}>
                        {discomType}
                      </option>
                    ))}
                  </Select>
